...
namespace Forecast.Method.AverageBased
{
  public class HoltHelper
  {
    ...
    public static double TrendValue(
      double trendSmoothingFactor, double smoothedValue,
      double previousSmoothedValue, double previousTrendValue)
    {
    ...
    public static double Forecast(
      double smoothedValue, double trendValue, int index)
    {
      return smoothedValue + index * trendValue;
    }
    //Starting trend is 1
    public static (List<double>, List<double>) CalculteSmoothedAndTrendValues(
      List<double> realValues, double dataSmoothingFactor,
      double trendSmoothingFactor, List<double> smoothedValues,
      List<double> trendValues, bool firstRunFlag = true)
    {
      ...
      if (firstRunFlag)
      {
        smoothedValues.Add(
          SmoothedValue(
            dataSmoothingFactor, realValues.First(), realValues.First(), 1));
        trendValues.Add(0.0);
      }
      else
      {
        smoothedValues.Add(
          SmoothedValue(
            dataSmoothingFactor, realValues.First(), smoothedValues.Last(),
            trendValues.Last()));
        trendValues.Add(
          TrendValue(
            trendSmoothingFactor, smoothedValues.Last(),
            smoothedValues.ElementAt(smoothedValues.Count - 2),
            trendValues.Last()));
      }
